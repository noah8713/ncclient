...
class NCElement(object):
    ...
    def remove_namespaces(self, rpc_reply):
        ...
        #print 'testing rpc_reply is %s' % (str(rpc_reply))
        #print 'parsing huge_tree %s' %str(self.__transform(etree.parse(StringIO(str(rpc_reply)), parser=p)))
        #print 'parsing huge_tree %s' %(etree.parse(StringIO(str(rpc_reply))))
        #self.__root = etree.fromstring(str(self.__transform(etree.parse(StringIO(str(rpc_reply))))))
        #ck = str(self.__transform(etree.parse(StringIO(str(rpc_reply)))))
        #print 'ck is %s' %ck
        #self.__root = etree.fromstring(str(self.__transform(etree.parse(str(rpc_reply)))))
        #self.__root = etree.fromstring(ck)
        try:
            self.__root = etree.fromstring(str(self.__transform(etree.parse(StringIO(str(rpc_reply)),
                                                                            self.__parser))),
                                           self.__parser)
